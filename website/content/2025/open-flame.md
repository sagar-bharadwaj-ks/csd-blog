...
It is 2040, and you're a hungry and busy PhD student in your office who has to attend a talk in thirty minutes. You DoorDash from your favorite restaurant. An autonomous robot picks up your order from a restaurant in a mall. It then drives itself from the mall to your university, finds your office, and hand-delivers it to you. You eat and then use your Augmeted Reality (AR) glasses to find your way to the conference room, in time, to attend the talk.

There are many pieces of technology that need to come together to enable such a future, such as, robotics, display technology, autonomous navigation, and so on. A largely ignored piece of the puzzle is the *mapping infrastructure* -- a system assigning labels to spaces and describing relationships between them. Notice that in the above scenario, the robot and the AR glass use maps of multiple locations -- the mall, the public roads leading from the mall to the university, and the university's indoor maps. 

But wait! There are plenty of mapping services already, e.g., Google Maps, Apple Maps, Mapbox, OpenStreetMap etc. "So what exactly is missing?", you might ask. Existing mapping infrastructure is centralized, i.e., they are hosted and maintained by single organizations. For example, Google Maps is hosted and served by Google. Only information gathered and exposed by organizations maintaining these maps is available to applications. Private organizations such as malls and universities are not inclined to upload their maps to centralized repositoiries maintained by third-parties. Indoor map data is sensitive and ownership of such data is a primary concern of private organization. Furthermore, philosophically, we do not want to live in a world where a large part of future -- physically-based digital infrastructure (e.g., robotics, augmented reality) -- is controlled by a few large corporations. 

Our work, [OpenFLAME](https://www.open-flame.com/), proposes a systems infrastructure where disparate organizations can maintain maps of their own spaces and serve location-based services to applications. In the above scenario, the university, for example, would maintain its own map in its own servers. A robot wishing to navigate within a university would request access to the university map servers and use its services. For example, it might use a *localization* service to determine where it is within the university campus and the *navigation* service to determine where to go. (comment: Write a line about fine-grained access control -- the robot does not have to see the whole map, but rather only some services). 

We borrow inspiration from the Web infrastructure to build OpenFLAME. The Web has many parallels to the our mapping problem. On the Web, independent organizations host their own websites and maintain their own access control policies. Similarly, we want maps to be hosted independently by organizations for their own spaces giving them complete data ownership and access control. The Web has a "discovery mechanism" in the DNS. That is, given a human readable name of a website, the DNS identifies the servers that host the website. Maps would need a similar discovery mechanism -- given a location, we need a system to identify all the maps hosted at that location.


# Design of OpenFLAME

<center><figure><img src="./design.png" width="400" alt="Design of OpenFLAME showing map servers and OpenFLAME client."><br> <figcaption><b>Figure 1.</b> OpenFLAME architecture.</figcaption></figure></center>

Figure 1 shows the simplified architecture of OpenFLAME. Maps of different regions are stored on separate *map servers* maintained by independent organizations. Map servers also provide location-based services on top of the maps that they store. An OpenFLAME client discovers potentially zero or more map servers covering the region of interest using a federated spatial database. It then contacts these discovered servers to obtain services such as routing and localization, combining results from multiple maps when needed.
...
**Map services.** Location-based services built on top of maps are called map services.  The green boxes in Figure 1 show some examples of map services. The tile service, for example, returns a visual representation of the map. Clients access map data only through map services.
...
<center><figure><img src="./data-organization.png" width="500" alt="OpenFLAME's data organization and query model."><br> <figcaption><b>Figure 2.</b> OpenFLAME's data organization and query model.</figcaption></figure></center>
...
<center><figure><img src="./s2IndexToGeodomain.png" width="300" alt="Converting S2 cell index to geo-domain."><br> <figcaption><b>Figure 4. </b>Converting S2 cell index to geo-domain.</figcaption></figure></center>

The process of converting an S2 cell index to a geo-domain is shown in Figure 4. The S2 cell index encodes the full hierarchy of the cell up to the top-level face which we convert to a DNS-compatible format. The geo-domain is organized such that the top-level face is at the right end and the smallest cell is at the left end of the domain name. We need a *domain suffix* to represent the root domain of the geo-domain. In our examples, we use `.loc`. The altitude of the S2 cell, rounded up to the nearest integer in feet, is prefixed to the geo-domain. If the altitude is unknown, the letter `U` is prefixed instead.
...
<center><figure><img src="./genGeoDomainsDiscovery.png" width="400" alt="Generating geo-domains for discovery."><br> <figcaption><b>Figure 5.</b> Generating geo-domains for discovery.</figcaption></figure></center>

1. We use S2's interior region coverer algorithm to get a set of S2 cells that cover the 2D geographic region. We call these the *base S2 cells* (Figure 5b).
2. For each base S2 cell, its corresponding geo-domain is generated (Figure 5c). We call these geo-domains the *base geo-domains*. 
3. For each base geo-domain, we generate all the parent domains by removing sub-domains sequentially from the left. We retain the altitude. For example, the parents of the domain `U.1.3.5.loc` are `U.3.5.loc`, and `U.5.loc`. (Figure 5d). Querying parents is essential to discover maps whose coverages are larger than the queried region. Optionally, we also add the children of base geo-domains upto a predefined level.
